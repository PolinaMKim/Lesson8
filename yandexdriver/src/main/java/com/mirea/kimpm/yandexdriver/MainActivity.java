...
import com.mirea.kimpm.yandexdriver.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity implements DrivingSession.DrivingRouteListener,
    ...
    private final Point ROUTE_END_LOCATION = new Point(55.762145, 37.634470);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(isWork) {
            ...
            final Point userLoc = new Point(55.633419, 37.527229);
            submitRequest(userLoc);

            PlacemarkMapObject marker = mapView.getMap().getMapObjects().addPlacemark(new
                            Point(55.762145, 37.634470),
                    ImageProvider.fromResource(this, R.drawable.point),
                    new IconStyle().setScale(0.05f)); // Scale the icon down to 5%
            marker.addTapListener(new MapObjectTapListener() {
                @Override
                public boolean onMapObjectTap(@NonNull MapObject mapObject, @NonNull Point point) {
                    Toast.makeText(MainActivity.this, "Зеленый викинг (там классная керамика)", Toast.LENGTH_SHORT).show();
                    return false;
                }
            });
        } else {
